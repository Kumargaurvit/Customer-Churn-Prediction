...
try:
    CreditScore = st.number_input("Enter your Credit Score", min_value=0, max_value=900)
    Country = st.selectbox("Select your Country", options=["France","Spain","Germany"], index=None)
    ...
    Balance = st.number_input("How much money do you currently have in the bank account?", min_value=0)
    NumOfProducts = st.number_input("How many bank products you currently use?", min_value=0, max_value=10)
    ...
    EstimatedSalary = st.number_input("What is your estimated salary? (Monthly)", min_value=0)
